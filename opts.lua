...
function M.parse(arg)
    ...
    cmd:option('-cache', '/data/model/LRCN/', 'subdirectory in which to save/log experiments')
    cmd:option('-data', '/data/db/UCF-101', 'Home of ImageNet dataset')
    ...
    cmd:option('-rnnSize',          4096,    'size of hidden node in LSTM')
    cmd:option('-nClasses',          101,    'number of classes in the dataset')
    ------------- Training options --------------------
    cmd:option('-nEpochs',             55,    'Number of total epochs to run')
    cmd:option('-epochSize',  2400000/8/32/2,    'Number of batches per epoch(total number / stride / batchSize)') 
    ...
    cmd:option('-trainType',   'finetune', 'none | transfer | finetune')
